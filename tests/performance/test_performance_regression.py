...
""" * 5  # Repeat 5 times for medium size
...
""" * 10  # Repeat 10 times for large size
...
class TestChunkingPerformance:
    ...
    def test_small_document_performance(self, default_config):
        ...
        metrics = measure_operation(
            lambda: chunker.chunk(SMALL_DOC),
            iterations=10
        )
    ...
    def test_medium_document_performance(self, default_config):
        ...
        metrics = measure_operation(
            lambda: chunker.chunk(MEDIUM_DOC),
            iterations=5
        )
    ...
    def test_large_document_performance(self, default_config):
        ...
        metrics = measure_operation(
            lambda: chunker.chunk(LARGE_DOC),
            iterations=3
        )
    ...
    def test_validation_overhead(self, default_config, no_validation_config):
        ...
        with_validation = measure_operation(
            lambda: chunker_with_validation.chunk(MEDIUM_DOC),
            iterations=5
        )
        
        # Measure without validation
        without_validation = measure_operation(
            lambda: chunker_without_validation.chunk(MEDIUM_DOC),
            iterations=5
        )
        
        # Calculate overhead
        overhead = (with_validation["mean"] - without_validation["mean"]) / without_validation["mean"]
...
class TestHierarchicalPerformance:
    ...
    def test_hierarchical_small_document(self, config):
        ...
        metrics = measure_operation(
            lambda: chunker.chunk_hierarchical(SMALL_DOC),
            iterations=10
        )
    ...
    def test_hierarchical_medium_document(self, config):
        ...
        metrics = measure_operation(
            lambda: chunker.chunk_hierarchical(MEDIUM_DOC),
            iterations=5
        )
    ...
    def test_hierarchical_large_document(self, config):
        ...
        metrics = measure_operation(
            lambda: chunker.chunk_hierarchical(LARGE_DOC),
            iterations=3
        )
    ...
    def test_navigation_performance(self, config):
        ...
        get_chunk_metrics = measure_operation(
            lambda: result.get_chunk(result.root_id),
            iterations=100
        )
        
        # Measure get_children
        get_children_metrics = measure_operation(
            lambda: result.get_children(result.root_id),
            iterations=100
        )
        
        # Measure get_flat_chunks
        get_flat_metrics = measure_operation(
            lambda: result.get_flat_chunks(),
            iterations=100
        )
