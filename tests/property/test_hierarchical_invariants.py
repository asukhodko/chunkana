...
def markdown_document(draw):
    ...
    for level in range(1, 4):  # H1, H2, H3
        ...
        for i in range(count):
            header_text = draw(text(min_size=5, max_size=50, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'Zs'))))
    ...
    for _ in range(paragraph_count):
        paragraph = draw(text(min_size=20, max_size=200, alphabet=st.characters(whitelist_categories=('Lu', 'Ll', 'Nd', 'Zs', 'Po'))))
        ...
        **Validates: Requirements 1.3**
        ...
        for chunk in result.chunks:
            ...
            if parent_id:
                ...
                assert parent_chunk is not None, f"Chunk {chunk_id} references non-existent parent {parent_id}"
            ...
            for child_id in children_ids:
                ...
                assert child_chunk is not None, f"Chunk {chunk_id} references non-existent child {child_id}"
        ...
        **Validates: Requirements 1.3, 1.5**
        ...
        for chunk in result.chunks:
            ...
            if is_root:
                ...
                assert parent_id in chunk_map, f"Chunk {chunk_id} references non-existent parent {parent_id}"
...
"""
        ...
        if result.chunks:
            ...
            strict_builder = HierarchyBuilder(
                validate_invariants=True,
                strict_mode=True
            )
...
"""
        ...
        if result.chunks:
            ...
            from chunkana.hierarchy import HierarchyBuilder
            non_strict_builder = HierarchyBuilder(
                validate_invariants=True,
                strict_mode=False
            )
