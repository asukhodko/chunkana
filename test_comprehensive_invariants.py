...
"""
    ...
    for chunk in result.chunks:
        ...
        if is_leaf != expected_is_leaf:
            errors.append(f"Chunk {chunk_id}: is_leaf={is_leaf}, children_count={len(children_ids)}")
        ...
        for child_id in children_ids:
            ...
            if not child_chunk:
                ...
                if child_parent_id != chunk_id:
                    errors.append(f"Child {child_id}: parent_id={child_parent_id}, expected {chunk_id}")
...
def test_error_handling():
    ...
    try:
        ...
        # Verify fixes
        if parent_chunk.metadata["is_leaf"] != False:
            print("✗ Parent is_leaf not fixed")
            return False
        
        if child_chunk.metadata["is_leaf"] != True:
